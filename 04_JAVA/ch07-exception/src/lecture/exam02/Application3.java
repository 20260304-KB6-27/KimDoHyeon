...
public class Application3 {
    ...
    public static void main(String[] args) {
        ...
        // 다중 catch 불럭 작성 가능 => 각 상황별 예외처리 구문 써두기
        ...
        try {
            ...
            int[] arr = new int[size];
            System.out.println(arr[inputIndex]);
        } catch (ArrayIndexOutOfBoundsException e){ // RuntimeEx의 하위 예외이기 때문에 상단에 먼저 써야 함. 그래야 먼저 탐색 됨.
            System.out.println("부적절한 인덱스 제시");
        } catch(RuntimeException e) { // 매개변수 다형성
            System.out.println("치환 불가 또는 음수제시 불가");
        }
        // 하위 예외 클래스 타입 아래에 상위 예외 클래스 타입 있어야 함. (순서 중요)
